<template>
  <div class="my-transactions-view">
    ...
    <div class="grid grid-3 mb-4">
      <div class="card stat-card">
        <h3>Total Spent</h3>
        <div class="stat-value">{{ settingsStore.formatPrice(totalSpent) }}</div>
      </div>
      <div class="card stat-card">
        <h3>Total Refunds</h3>
      ...
      <div v-for="transaction in filteredTransactions" :key="transaction.id" class="card transaction-card">
        <div class="transaction-header">
          <div>
            ...
            <p class="transaction-tour">Guide: {{ getGuide(transaction)?.name }}</p>
          </div>
          <div class="transaction-amount" :class="transaction.type">
            <span v-if="transaction.type === 'refund'">+</span>
            <span v-else>-</span>
    ...
    <div v-if="selectedTransaction" class="modal-overlay" @click="selectedTransaction = null">
      <div class="modal-content card" @click.stop>
        ...
        <div class="detail-grid">
          ...
          <div class="detail-row">
            <strong>Guide:</strong>
            <span>{{ getGuide(selectedTransaction)?.name }}</span>
          </div>
          <div class="detail-row">
...
const myTransactions = computed(() => {
  return dataStore.getTransactionsByUser(authStore.user.id)
    .filter(t => t.touristId === authStore.user.id) // Only show transactions where user is the tourist
...
const filteredTransactions = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    transactions = transactions.filter(t => 
      ...
      getGuide(t)?.name.toLowerCase().includes(query)
    )
  }
...
function getGuide(transaction) {
  return dataStore.getUserById(transaction.guideId)
}

function getTypeColor(type) {
  return type === 'payment' ? 'danger' : 'success'
}
