      ...
      <div v-for="dispute in filteredDisputes" :key="dispute.id" class="card dispute-card">
        <div class="dispute-header">
          <div>
            ...
            <p class="dispute-roles">
              <span v-if="dispute.respondentId === authStore.user.id" class="role-badge against-you">
                🔴 Filed Against You
              </span>
              <span v-else class="role-badge filed-by-you">
                🟢 Filed By You
              </span>
              <span class="vs-label">
                {{ dispute.complainantId === authStore.user.id ? 'Against' : 'By' }}: 
                <strong>{{ getOtherUser(dispute)?.name }}</strong>
              </span>
            </p>
          </div>
        ...
        <div v-else-if="dispute.status === 'under_review'" class="dispute-info-message">
          <small>ℹ️ This dispute is currently being reviewed by our admin team.</small>
        </div>
        
        <div v-else-if="dispute.status === 'pending'" class="dispute-info-message">
          <small>⏳ This dispute has been submitted and is waiting for admin review.</small>
        </div>
...
const myDisputes = computed(() => {
  return dataStore.disputes
    .filter(d => d.complainantId === authStore.user.id || d.respondentId === authStore.user.id)
...
function getOtherUser(dispute) {
  const otherUserId = dispute.complainantId === authStore.user.id 
    ? dispute.respondentId 
    : dispute.complainantId
  return dataStore.getUserById(otherUserId)
}
